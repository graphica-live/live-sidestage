...
export function MatchManager({
  ...
  const placementChoices = useMemo(
    () => (seed.length < 2 ? [] : placementOptions(seed.length, bracketMethod)),
    [seed, bracketMethod]
  );
  ...
  const plannedPlacementRounds = useMemo(
    () =>
      seed.length < 2 || plannedPlacementDepth === 0
        ? []
        : placementRounds(
            buildPlacementBlocks(seed.length, bracketMethod, plannedPlacementDepth),
            roundCount
          ),
    [seed, bracketMethod, plannedPlacementDepth, roundCount]
  );
