 ...
 *
 * **battleId だけで引き、行の hostTiktokUid をそのまま使う。** participant 由来の roomId は介さない
...
export async function loadTapPointsForBattle(battleId: string): Promise<BattleTapPointInput> {
  const [rows, battles] = await Promise.all([
    prisma.tiktokBattleTapPoint.findMany({
      where: { battleId },
      // **hostTiktokUid(タップの宛先の配信者)を読む。** 同テーブルの tiktokUid は
      // 10タップに到達した「リスナー」で、anchor 単位の差し引きには使えない。
      select: { roomId: true, hostTiktokUid: true, occurredAt: true, points: true },
      orderBy: [{ occurredAt: "asc" }, { hostTiktokUid: "asc" }],
    }),
  ...
  const tapTrackedTiktokUids = new Set<string>();
  for (const row of rows) {
    tapPoints.push({ tiktokUid: row.hostTiktokUid, occurredAt: row.occurredAt, points: row.points });
    if (trackedRoomIds.has(row.roomId)) tapTrackedTiktokUids.add(row.hostTiktokUid);
  }
