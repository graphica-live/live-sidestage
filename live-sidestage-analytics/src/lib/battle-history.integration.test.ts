...
describe("queryBattles opponent.count", () => {
  it("バトルごとの相手room数を返す(期間全体で観測した他room数の合算にならない)", async () => {
    ...
    const { battles } = await queryBattles(selfRoomId, range);
    ...
    expect(battle1.opponent).toEqual({ tiktokId: OPPONENT_B_TIKTOK_ID, count: 1 });
    expect(battle2.opponent).toEqual({ tiktokId: OPPONENT_C_TIKTOK_ID, count: 1 });
  });
});
