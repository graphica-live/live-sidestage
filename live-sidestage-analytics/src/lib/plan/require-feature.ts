 ...
 */
export async function hasFeatureAccess(principalId: string, feature: FeatureKey): Promise<FeatureAccessResult> {
...
/**
 * principalIdがfeatureを利用できなければ403のNextResponseを返す(利用できれば null)。
 *
 ...
 */
export async function requireFeature(principalId: string, feature: FeatureKey): Promise<NextResponse | null> {
  const { allowed } = await hasFeatureAccess(principalId, feature);
