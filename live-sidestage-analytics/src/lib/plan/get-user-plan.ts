...
// 同一principalIdが複数provider(Stripe/Google Play/Apple)で有効な行を持ちうるため、
// 有効な全行から最上位プランを算出する。
export async function getUserPlan(principalId: string): Promise<PlanTier> {
  // entitlementActive:trueだけでなく、バックフィル未実行の旧Stripe行(provider未設定)も
  // 拾う必要があるため、principalId一致の全行を読んでisEntitlementRowValidで判定する。
  const [subscriptions, ambassador] = await Promise.all([
    ...
    prisma.ambassador.findUnique({ where: { principalId }, select: { id: true } }),
  ]);
  const now = new Date();
