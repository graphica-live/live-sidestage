...
describe("computeLeaseWindow", () => {
  ...
  it("終了が遠すぎる場合は上限まで切り詰め、本来の期限は保持する", () => {
    // 上限(120日)を確実に超える終了日時
    const endAt = new Date(now.getTime() + 200 * DAY_MS);
    const w = computeLeaseWindow(endAt, now);

    expect(w.clamped).toBe(true);
    expect(w.granted.getTime()).toBe(now.getTime() + MAX_LEASE_DAYS * DAY_MS);
  ...
  it("猶予を足した結果がちょうど上限なら切り詰めない", () => {
    ...
    const endAt = new Date(now.getTime() + MAX_LEASE_DAYS * DAY_MS - LEASE_GRACE_MS);
    const w = computeLeaseWindow(endAt, now);
  ...
  it("上限を1ミリ秒でも超えたら切り詰める", () => {
    const endAt = new Date(
      now.getTime() + MAX_LEASE_DAYS * DAY_MS - LEASE_GRACE_MS + 1
    );
    const w = computeLeaseWindow(endAt, now);
