...
export async function resolveStreamerByApiKey(req: NextRequest): Promise<{ id: string; roomId: string | null } | null> {
  ...
  //
  // **apiKeyの一致はTikTokアカウント所有の証明ではない。** tiktokHandleの登録は無条件・
  // 重複可(他人のtiktokHandleを登録すれば同じroomのapiKeyを誰でも取得できる)なので、
  // verifiedを課しても防御にはならない。ここで露出するデータ(月間MVP/TOP5の
  // tiktokHandle・nickname・avatar・コイン数)は、session認証の既存API(gifts等)でも
