...
> 終わったバトルは、勝敗の自動判定にもバトル倍率にも入らない（日程内のギフト自体は
> 順位には入る）。日程の終了時刻は進行の遅れを見込んで余裕をもって設定すること。
> 日程設定画面にも同じ案内を出している。
...
検知したバトル（`DetectedBattle`）は破棄しても消えない。同じ日程に組み直せば、作り直した表でも
同じバトルが検知され直す。
