...
イベント作成後いつでもやり直せる（方式を変えることもできる）。ただし**作り直しは
「破棄 → 作成」の2手順**で、表がある状態のまま組み替えることはできない。
...
公開ページのバトルスコアにも入らない（バトル中に飛んだギフトは通常のギフトとして順位表には
入る）。誤って開始してすぐ切り上げたバトルが、同じ日程の本番のバトルの検知を邪魔しなくなる。
