...
入力させて確認する（消える結果の件数と内訳をその場で出す）。参加者が2組未満に減った、
日程を縮めて全ラウンドが収まらない等で作り直せない状態でも、「破棄だけする」で古い表を
消せる。破棄と再作成は同じトランザクションなので、作り直しが失敗して表を失うことはない。

検知したバトル（`DetectedBattle`）は破棄しても消えない。時間枠が同じなら、作り直した表でも
同じバトルが検知され直す。
