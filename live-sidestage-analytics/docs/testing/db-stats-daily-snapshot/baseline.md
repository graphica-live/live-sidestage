...
前日比+閾値%超の増分があればLINE Messaging APIへ通知する機能。スケジューリング(いつ・何回
...
| TC-DBS-007 | 異常なしのときのメッセージ整形 | `formatDbStatsMessage` | 正常 | `anomalies`が空 | 「異常な増分はありません。」を含み、テーブル数・件数合計・サイズ合計を出す | `npm run test:unit`(`src/lib/db-stats/message.test.ts`) | PASS | |
| TC-DBS-008 | 異常ありのときのメッセージ整形 | `formatDbStatsMessage` | 正常 | `anomalies`に1件(pctChange=0.26) | 「⚠️」と`schema.table: prev → today (+26%)`形式の行を含む | `npm run test:unit` | PASS | |
...
| TC-DBS-010 | LINE認証情報未設定時は送信をスキップする | `sendLineMessage` | negative | `LINE_CHANNEL_ACCESS_TOKEN`/`LINE_ALERT_TARGET_ID`いずれか未設定 | 例外を投げず、warnログを出して終了する(呼び出し元のtickを止めない) | 自動テストなし | NOT RUN: 自動テストなし(外部API呼び出しのため) | 2026-09-06、`DB_STATS_FORCE_RUN=1`でevent-worker.tsを実際に起動し、未設定時の警告ログ出力を実機確認済み |
| TC-DBS-011 | LINE送信失敗時もエラーを投げない | `sendLineMessage` | negative | LINE APIが非2xxを返す、またはfetch自体が例外を投げる | 例外を投げず、errorログを出して終了する | 自動テストなし | NOT RUN: 自動テストなし(外部API呼び出しのため。実装はtry/catchで両経路とも握りつぶす) | |
...
- LINE Messaging APIの実送信結果そのもの(実チャネル・実送信先が無いと検証できないため、ユーザーがRailway側に認証情報を設定した後の本番疎通確認に委ねる)
