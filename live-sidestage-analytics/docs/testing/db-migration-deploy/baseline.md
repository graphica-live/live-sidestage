...
| TC-MD-002 | migrate deploy後のスキーマがPrisma schema setと完全一致する（drift無し） | `0_init`＋Wave1-B migration適用後のDB | 回帰 | TC-MD-001実施済みDB | `migrate diff --from-schema-datasource --to-schema-datamodel --exit-code`がexit 0（`No difference detected.`） | `DATABASE_URL=<専用DB> npx prisma migrate diff --from-schema-datasource prisma --to-schema-datamodel prisma --exit-code` | PASS | mainのWave1削除4項目（`room_monitor_leases`テーブル・両`hostDisplayIds`・`scheduledStartAt`/`scheduledEndAt`）が再生成`0_init`に反映されていることの確認を兼ねる |
| TC-MD-004 | CLI loads the full schema set from package.json prisma.schema directory | prisma/*.prisma | normal | DATABASE_URL form only | npx prisma validate prints The schemas at prisma are valid | DATABASE_URL=postgresql://build:build@localhost:5432/build npx prisma validate | PASS | 2026-09-13 multi-file split |
