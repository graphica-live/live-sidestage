...
| TC-BRD-038 | 確定時に元 Gift の `giftPictureUrl` が allowlist 済みなら giftEvent へ写る | `computeBattleSnapshot` / `commitBattleSnapshot` | 正常 | TikTok CDN の `giftPictureUrl` を持つギフト1件 | `battle_history_gift_events.giftPictureUrlSnapshot` が一致。javascript: は null | `[itg]` | 実装後 | コミュニティギフト再生の正本。カタログには書かない |
