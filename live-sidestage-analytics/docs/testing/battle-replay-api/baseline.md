...
| TC-BRA-044 | ギフト画像は `giftPictureUrlSnapshot` を優先し、無ければカタログ。不正URLは null | `buildPayload` | 正常/データ欠損/セキュリティ | snapshot+catalog / snapshotのみ / catalogのみ / javascript: URL | snapshot URL / snapshot URL / カタログ URL / `img: null` | `[unit]` | PASS | 再生は当時の受信画像が正。カタログは旧行の控え |
...
| TC-BRA-042 | itemCardEvents become itemEvents {t,a,k} sorted by time; no sender fields | buildPayload | normal | self glove t=40s, opponent vault t=45s, self hammer t=50s | [{40000,0,2},{45000,1,12},{50000,0,6}] | [unit] | PASS | public payload also omits sender |
| TC-BRA-043 | finalized replay payload includes observed item uses | queryBattleReplay | normal | TiktokBattleItemUse glove at +40s on self host | itemEvents=[{t:40000,a:0,k:2}] | [itg] | PASS | copied at finalize onto target participant |
