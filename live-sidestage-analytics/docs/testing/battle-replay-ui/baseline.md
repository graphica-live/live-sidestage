...
| TC-BRU-057 | 終了直後、勝者側に大きく WIN が出てから常設の小バッジ位置(配信者アイコンの真上、`.replay-ripple` 基準)へ移動・縮小する。着地位置はレイアウト(1vs1/チーム戦2vs2/1vs3/3人コラボ/4人コラボ)に依らず共通。演出の実時間長は再生速度に依存せず、既定速度(4倍速)を含むどの速度でも常設位置まで動き切る | `ReplayCell` の `winRevealPhase` / `useReplayClock` の `END_FADE_MS` | 正常/回帰/境界 | 勝者側セルを末尾直前からシークし、200ms刻みで観測。1x/2x/4x それぞれで確認。duo/quad/trio(自分win・相手win)/team22/one3(自分win・相手win)の全レイアウトで確認 | 終了(`elapsedMs>=durationMs`)から実時間900msで `.replay-ripple` 中央(アバター中心)に大きく出現(opacity/scaleが単調増加)、続く実時間650msで `.replay-ripple` 上端(アバター真上、少し重なる)へ移動しながら縮小、以後は既存の小さい `.replay-win` バッジ（`font-size: 14px`）のみに収束する。中央大表示は `.replay-win-big`（`font-size: 34px`）。この実時間の長さ(900ms/650ms)はどの再生速度でも一定（`WIN_REVEAL_MS`/`WIN_TRAVEL_MS` を `motionScale` で割って算出するため）。演出が完走する前に再生が自動停止して途中の位置(配信者アイコンに重なった状態)のまま固定表示されることはない。一時停止・シークでも同じ計算式で位置が決まる（`elapsedMs` の純関数） | `[pw]` `[inject]`(team22/one3 は実データに無いため teams 配列を再構成して模擬) | PASS(2026-09-08 実測: 左側セル 中央 opacity 0.12→0.86 / 移動後 top 48%→12%, left 51%→88%, scale 0.98→0.47。2026-09-10 既定4倍速で `getBoundingClientRect()` 実測し `.replay-win` 着地後がアバターと交差しないことを確認。同日、着地位置を「セル座標系の隅固定 px」から「`.replay-ripple` 基準の相対配置」へ変更し、duo/quad/trio×2/team22/one3×2 の全7レイアウトでスクリーンショット実測、いずれもアバターの真上に統一されることを確認) | ひっそり小バッジが出るだけでは勝敗が分かりにくいというユーザー指摘への対応。以前は演出の尺が elapsedMs 単位の固定値で、再生速度が上がるほど実時間で見える長さが短くなっていた（速度4倍で実質225ms）ため修正。2026-09-10、既定速度(4倍速)で `END_FADE_MS`(elapsedMs単位で固定4200)が WIN 演出の所要尺(4倍速で6200相当)より短く、演出完了前に再生が自動停止してアバターに重なった中途半端な位置で固定表示される回帰を発見・修正（`END_FADE_MS * speed` で実時間の猶予を一定にした）。さらに同日、着地位置がセル座標系の px 固定だったため、アバターの実サイズ・セル縦横比がレイアウトごとに違い「アバターの真上」がレイアウトによってズレる／セルによってはクリップされて見えない問題があり、アバター基準の相対配置へ変更 |
| TC-BRU-061 | 常設 WIN バッジ・演出中の WIN バッジは、どのレイアウトでも順位バッジ・名前チップと重ならない | `ReplayCell` / `globals.css` の `.replay-win` | 回帰/境界 | duo/quad/trio(自分win・相手win)/team22/one3(自分win・相手win)それぞれで勝者側セル(左側・右側とも)を終了後(`elapsedMs>=durationMs`)へシーク | WIN バッジ(配信者アイコン真上)と順位バッジ(セル左下)・名前チップ(セル右下)が別位置に表示され重ならない。左側セル・右側セルのどちらが勝者でも同じ基準(アバター真上)で表示される | `[pw]` `[inject]`(team22/one3 は実データに無いため teams 配列を再構成して模擬) | PASS(2026-09-10 実測、duo/quad/trio×2/team22/one3×2 の全7レイアウトでスクリーンショット確認) | 以前は `.replay-win` がセル座標系の固定位置(px 指定の隅)で、`cell.right` の値によって別途 CSS 上書きが必要だった。アバター基準の相対配置にしたことで cell.right を考慮する特別処理が不要になった |
| TC-BRU-062 | チーム戦(2vs2・1vs3)で勝者陣営に複数人いる場合、メンバー全員に WIN バッジが表示される | `ReplayStage` の `winningTeamIndex` 判定(`cell.teamIndex === winningTeamIndex`) | 正常/境界 | team22(2vs2)で自陣営(2人)が勝者のケース、one3(1vs3)で相手陣営(3人)が勝者のケースをそれぞれ終了後へシーク | team22 は自陣営の2セルとも WIN バッジが表示される。one3 は相手陣営の3セルとも WIN バッジが表示される。負けた陣営のセルには WIN バッジが出ない | `[pw]` `[inject]`(実データに team22/one3 が無いため、4人コラボの teams 配列を2陣営(2人+2人 / 1人+3人)に再構成して模擬) | PASS(2026-09-10 実測: team22 自陣2セル・one3 相手陣営3セルとも WIN バッジ表示を確認) | 判定ロジック自体は陣営単位(`teamIndex` 一致)で以前から正しかったが、着地位置がセル座標系の px 固定だったため、セルの縦横比・アバターサイズによっては表示位置がセル外へはみ出てクリップされ見えないケースがあった(アバター基準の相対配置への変更で解消) |
| TC-BRU-063 | itemEvents inside 30s window appear on the matching scorebar side with remaining time | itemsAt / ReplayScoreBar | normal | glove at t=10s on self, hammer at t=12s on opponent, elapsed=15s | left chip remainMs=25s, right chip remainMs=27s; outside window hidden | [unit] | PASS | ITEM_EFFECT_MS=30000. sides from layout.cells.right |
| TC-BRU-064 | each side keeps at most 5 newest item chips | itemsAt | boundary | 7 overlapping gloves on self | keys i2..i6 remain | [unit] | PASS | |
| TC-BRU-065 | 2位・3位ブースターは同色の黄。出現直後だけ発光する | ReplayScoreBar / itemAppearT / globals.css | 正常/境界 | top2 と top3 を効果開始(remainMs=30000)と 520ms 後で比較 | どちらも背景 #f5c451・文字 #0d0f13。開始時 --replay-item-appear=1、520ms 後は 0。reduced-motion では scale/filter/追加shadow なし | [unit] [vqa] | PASS | 以前は top2=#4d9fff / top3=#ffa64d で区別しづらく、出現の合図も無かった |
...
| TC-BRU-059 | 終了間際に発生した大ギフト演出・カード・リップルは、終了後も自然に消滅する（残留しない）。猶予は再生速度によらず実時間で一定(4200ms) | `useReplayClock` の `END_FADE_MS` | 境界/回帰 | 終了直前(リップルが表示中の位置)へシークし、猶予期間(実時間4200ms超)だけ実時間で待つ。1x/4x それぞれで確認 | 大ギフト演出・カード・リップル(`.replay-ring`)が猶予期間中に自然消滅し、要素数が0になる。演出が終了の瞬間の見た目のまま固まらない。停止判定は `durationMs + END_FADE_MS * speed`（elapsedMs単位）で行うため、速度が上がっても実時間の猶予(4200ms)は変わらない | `[pw]` | PASS(2026-09-08 実測: シーク直後 `ringCount:3` → 7秒後 `ringCount:0, bigGiftCount:0`。2026-09-10、`speed` 倍する修正後も1x/4xとも実時間約4.2秒で自然停止することを確認) | 即座停止(旧実装)では末尾間際の演出が消滅計算を完了できず画面に残り続けていた(ユーザー指摘)。2026-09-10、`END_FADE_MS` がelapsedMs単位の固定値のままだったため、既定速度(4倍速)ではWIN演出(TC-BRU-057)の所要尺より先に猶予が尽きて演出が完走前に打ち切られる回帰があり、`speed` 倍する形に修正 |
