...
- **BIO 認証（bio に認証コードを貼ってサーバーがスクレイピング確認）は、どの機能の前提にもせず UI から撤去済み**（2026-09）。`VerifyGate`・`BIO_VERIFICATION_GATE_ENABLED`・`VerifiedLockNotice`（mobile）・イベント参加者一覧の「本人確認済み」バッジはすべて撤去済みで、`verified` の値によらず全機能が利用できる。**`/api/verify/*`・`verificationCode`・`verified` 列自体は実装を残してある**（将来の復活に備えた明示的な判断）。`resolveStreamerByApiKey()`（`src/lib/api-auth.ts`）も `verified` を問わない
...
- `public` のテーブルを読むのは [src/event/analytics-db.ts](src/event/analytics-db.ts) だけ。raw SQL は multiSchema でも自動修飾されないので `public."TiktokRoom"` のように完全修飾する。**列は必ず明示する**（`SELECT *` は `Streamer.apiKey` や `Principal.password`（Prisma model。物理テーブル名は `"User"`）まで持ってくる）
