...
**`prisma/migrations/` は baseline を含めて本番で `prisma migrate deploy` で実行される（2026-09 以降）。** baseline migration（`0_init`）は開発時に `prisma migrate diff --from-empty --to-schema-datamodel schema.prisma` で生成し、本番では `prisma migrate resolve --applied 0_init` で「適用済み」登録される。その後のスキーマ変更は `prisma migrate dev` で通常のmigrationファイルを生成し、本番は `migrate deploy` で順序通り実行される。今後schema.prisma を変更するときは必ず `prisma migrate dev` で migrationファイルを作り、レビュー後に commit すること（build 段階では実行されず、Pre-Deploy Command で本番に反映される）。
