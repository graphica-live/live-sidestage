...
- **データモデルの肝**: 同一 `tiktokId` は `TiktokRoom` 1行 = TikTok 接続1本を複数の `Streamer`（登録ユーザー）で共有する。ギフト元データ `Gift` は不変。以前あった配信者ごとの手動編集・非表示（`GiftEdit`）は 2026-09 に UI・API・テーブルごと撤去した
