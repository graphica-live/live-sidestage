...
**`prisma/migrations/` は本番では一切実行されない（`db push` はmigrationsフォルダを読まない）。** そのため実運用上は схема変更に対してmigrationファイルを作らなくても本番は動く。ただしこの運用には構造的な穴がある——**db pushを実行するのはwebサービスの起動時だけ**なので、`WORKER_COUNT`変更やコード更新でworker系（worker1/2/3・event-worker・worker-guardian）がwebより先に新スキーマ依存のコードでデプロイされると、DBスキーマがまだ追いついておらずPrismaClientKnownRequestError(P2022 等)でクラッシュする窓がある（2026-09-04 worker3のhealthcheck失敗で実際に発生）。`prisma/schema.prisma`にモデル・カラムを足したら、たとえ本番には効かなくても`npx prisma migrate diff --from-url $DATABASE_PUBLIC_URL --to-schema-datamodel prisma/schema.prisma --script`等で差分を確認し、migrationファイルとして残しておくこと（履歴のドキュメントとして。実行はされない）。
