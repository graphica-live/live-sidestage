...
時計チップは絶対配置（`padding: 2px 9px` / `radius 999px` / `z-index: 4`）で、**残り時間のカウントダウン**（尺 5 分なら `05:00` → `00:00`）。経過時間のカウントアップではない。
位置は**バーの中ではなくバーの直下**（`left: 50%` / `top: 100%` / `transform: translate(-50%, 6px)`）。バーの中央に重ねるとセグメントのスコア数値と重なってどちらも読めない。
陣営グリッドの上端へ数 px 掛かるのは許容（配信者アイコンはセル中央なので隠れない）。
**自動早送り中はこのチップも点滅する**（`var(--replay-gold)` の文字色 + 同色 1px の内側リング + `opacity 1 ↔ 0.35` を 900ms 周期。`prefers-reduced-motion: reduce` では点滅なし・色だけ残す）。時計だけ速く進むので、色が変わらないと早送り中だと判らない。
...
配信者ブロックは**常にセルの縦中央**。アイコン 96px（`.lg` 128px / 1vs3 の相手枠 76px）・
**1vs1 は左右とも `.lg` 128px**（枠の広さが左右同じなので、自分だけ大きくすると非対称に見える。2026-09-07 にユーザー指示で変更。comp.png の 2コラボパネルは変更前の 128/96）。3コラボ・1vs3 は自陣が全高で相手枠が縦に狭いため、従来どおり自分だけ `.lg`。`border: 3px solid rgba(255,255,255,0.8)`・`radius 999px`、アイコンと個人スコアの間 `gap: 7px`。**枠には常に個人スコアを出す。**
...
**高額ギフトは、受け取った配信者枠にギフト画像を出す。額で2段階。**

| 段 | 条件 | 画像の大きさ | 配信者アイコン |
| --- | --- | --- | --- |
| `big` | 10,000コイン以上（`BIG_GIFT_MIN_DIAMONDS`） | 枠に対して `88%` | 隠す（`opacity: 0`） |
| `mid` | 1,000〜9,999コイン（`MID_GIFT_MIN_DIAMONDS`） | 枠に対して `44%`（`big` の半分） | **隠さない**（演出が枠の半分しか覆わないため） |

どちらも `object-fit: contain`、`drop-shadow(0 8px 22px rgba(0,0,0,0.6))`、`z-index: 2`（配信者ブロックより上・ギフトカードより下）。
尺 `BIG_GIFT_DURATION_MS = 3200ms`。`scale 0.74 → 1` で出て（先頭 12%）、`rotate ±2.4deg` + `translateY ±3px` の揺れ（周期 = 尺 ÷ 4、**実時間の CSS アニメで再生速度で割る**）を続け、末尾 26% で `scale 1.07` へ伸びながらフェードアウトする。出入り・揺れ・尺は2段で共通。
**出入りの `opacity` / `scale` は CSS アニメではなく `elapsedMs` から計算した inline 値**（描画は `elapsedMs` の純関数、という全体の設計に合わせる）。CSS アニメだと一時停止・シーク中に実時間で走り切り、`opacity: 0` の演出が枠に残ったまま配信者アイコンだけ消える。
**演出中は配信者アイコン（`.replay-host`）を `opacity: 0` で隠す**（180ms の transition。**`big` のみ**）。同じ枠で重なったら**上の段が優先**で、同じ段どうしなら新しい方を採る（`mid` の演出中に `big` が来たら `big` へ差し替わり、その逆は起きない）。ギフト画像が取れないギフトでは演出そのものを出さない（アイコンだけ消えるのを防ぐ）。
`prefers-reduced-motion: reduce` では揺れを止め、出入りのフェードだけ残す。
...
**ギフト画像には背景を敷かない**（透過PNGなので面を敷くと絵が潰れる）。`object-fit: contain`。画像が取れなかったときのプレースホルダ（`span.replay-thumb`）だけ `rgba(255,255,255,0.14)` の面を出す。
...
- **再生開始時の既定は 4x + 自動早送り ON。** 自動早送りはギフトカードが1枚も出ていない 5 秒以上の区間を選んだ速度の 4 倍で流し、次のカードの 1 秒前で等速へ戻す。**赤帯が出ている区間は飛ばさない**。早送り中は速度チップが**実効倍率**（4x なら `16×`）を `var(--accent)` の文字色 + `animate-pulse` で表示し、抜けたら選んだ速度へ戻る
