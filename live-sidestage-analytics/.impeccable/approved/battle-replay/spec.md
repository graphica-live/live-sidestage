...
**10,000コイン以上のギフト**（`BIG_GIFT_MIN_DIAMONDS`）は、受け取った配信者枠いっぱいにギフト画像を出す。
枠に対して `88%`（`object-fit: contain`、`drop-shadow(0 8px 22px rgba(0,0,0,0.6))`）、`z-index: 2`（配信者ブロックより上・ギフトカードより下）。
尺 `BIG_GIFT_DURATION_MS = 3200ms`。`scale 0.74 → 1` で出て（先頭 12%）、`rotate ±2.4deg` + `translateY ±3px` の揺れ（周期 = 尺 ÷ 4、**実時間の CSS アニメで再生速度で割る**）を続け、末尾 26% で `scale 1.07` へ伸びながらフェードアウトする。
...
**演出中は配信者アイコン（`.replay-host`）を `opacity: 0` で隠す**（180ms の transition）。同じ枠で重なったら新しい方を採る。ギフト画像が取れないギフトでは演出そのものを出さない（アイコンだけ消えるのを防ぐ）。
...
| 大ギフト演出 | 10,000コイン以上のギフト画像を枠いっぱいに出し、揺らしてフェードアウト | `giftEvents[].d` + `gifts[].img` |
