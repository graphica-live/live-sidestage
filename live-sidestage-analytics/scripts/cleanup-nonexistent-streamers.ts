// TikTok上に存在しないtiktokHandle(打ち間違い等)で登録されたStreamerを見つけて削除する
...
//
// **配信者がTikTokで改名(tiktokHandle変更)した場合、この判定だけでは「打ち間違いで最初から
// 存在しない」と区別できない — 改名後は旧tiktokHandleへの問い合わせが同じNOT_FOUNDになる。**
// 上のGift件数チェックが、既にギフトを受けていた改名済みアカウントの誤削除は防ぐ。
//
// TikTokのレスポンス(api-live/user/room/のdata.user)には永続的な安定ID `secUid` / `id`
// (数値tiktokUid)が含まれるが、どちらを鍵にした逆引き(→現在のtiktokHandle)も無認証では不可能と
// 実測済み(2026-08確認)。`id`(user_id)側もsecUidと全く同じ失敗パターンだった:
//   - api/user/detail/?secUid=... / ?user_id=... → いずれもHTTP 200だが本文が完全に空
//     (msToken等の署名必須API)
//   - api-live/user/room/?secUid=... / ?user_id=...(tiktokHandleなし) → params_error。
//     tiktokHandleが必須パラメータ
//   - 正しいsecUid/user_id + 存在しないtiktokHandle → user_not_found(どちらも検索条件に
//     一切使われない)
//   - node/share/user/@<tiktokHandle> / node/share/user/<数値id> → いずれも403 Forbidden(即ブロック)
...
async function resolveWithRetry(tiktokHandle: string): Promise<TiktokProfileResult> {
  for (let attempt = 0; ; attempt++) {
    const result = await fetchTiktokProfile(tiktokHandle);
    if (result.ok || result.reason !== "RATE_LIMITED" || attempt >= MAX_RATE_LIMIT_RETRIES) {
      return result;
    }
    console.log(`  @${tiktokHandle}: レート制限。${RATE_LIMIT_RETRY_MS / 1000}秒待って再試行...`);
...
async function main() {
  ...
  try {
    const streamers = await prisma.streamer.findMany({
      select: { id: true, tiktokHandle: true, principalId: true, roomId: true, createdAt: true },
    ...
    for (const streamer of streamers) {
      const normalized = normalizeTiktokId(streamer.tiktokHandle);
    ...
    for (const s of deleteCandidates) {
      console.log(`  ${s.id} @${s.tiktokHandle} (principalId=${s.principalId}, 登録=${s.createdAt.toISOString()})`);
    }

    if (needsReview.length > 0) {
      console.log(`\n=== 要手動確認(TikTok上に存在しないが受信ギフトあり、自動削除しない): ${needsReview.length}件 ===`);
      for (const { streamer: s, giftCount } of needsReview) {
        console.log(`  ${s.id} @${s.tiktokHandle} (principalId=${s.principalId}, ギフト${giftCount}件)`);
      }
    }
    ...
    if (inconclusive.length > 0) {
      ...
      for (const { streamer: s, reason } of inconclusive) {
        console.log(`  ${s.id} @${s.tiktokHandle} (${reason})`);
      }
    }
    ...
    for (const s of deleteCandidates) {
      try {
        ...
        console.error(`  @${s.tiktokHandle} の削除に失敗:`, err);
      }
    }
    console.log(
