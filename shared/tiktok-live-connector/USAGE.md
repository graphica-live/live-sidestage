...
live-sidestage 各アプリが `tiktok-live-connector` を消費するときの実装指示。
接続パターンは tikeffect 実装から抽出。重複排除キー（§14）は analytics 本番実測。

ライブラリ本体・fork 改造手順は [README.md](./README.md)。バトル固有は [BATTLE-EVENTS.md](./BATTLE-EVENTS.md)。
...
- [tiktok-super-fan-discrimination.md](../../live-sidestage-analytics/docs/tiktok-super-fan-discrimination.md)（analytics 側の socket 配信フィールド）
