...
- 認証フロー: `POST /api/mobile/auth/{google,apple,email/login}` → access token(短命JWT)+refresh token(rotation付き)を発行 → access tokenをそのままsocket.io接続(`auth: {token}`)にも使う。失効したら保存済みのrefresh tokenで`/api/mobile/auth/refresh`を叩いて無言再発行する(メイン/背景Isolateどちらからでも可能)
...
- Apple サインイン: **Android** は Custom Tab の web フロー（Services ID 必須、`--dart-define=APPLE_SERVICES_ID=...` 無しではボタン非表示）。**iOS** はネイティブ（entitlements 済みビルドではボタン既定表示）。共通で **authorizationCode** をサーバーへ送り、`state` / `nonce` を検証。手順は [README.md](README.md)
...
- `AuthSession.provider` は **どちらでログインしたかの記録**で、設定画面のアカウント表示（Google/Apple）に使う。保存済みセッションでは `provider` を**必須キーにしない**（既存インストールのセッションが消える）
