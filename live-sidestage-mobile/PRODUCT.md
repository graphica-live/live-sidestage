...
- ギフト名は `GET /api/mobile/gifts` が返す「直近に受け取ったギフト」から選ぶ。ピッカーは未入力なら全件を並べ、入力すると即時に絞り込む。コイン帯(1〜9 / 10〜99 / 100〜999 / 1000以上)でも絞り込める。
  - **これはTikTokの全ギフトカタログではない**。自分の部屋が過去に受け取ったギフトだけなので、まだ貰ったことのないギフトは一覧に出ない。そのため**一覧に無いギフト名はそのまま入力して登録できる**。
  - 全カタログを出すには、Worker側の `TikTokLiveConnection.availableGifts` を永続化して配信する仕組みが要る(未実装)。
