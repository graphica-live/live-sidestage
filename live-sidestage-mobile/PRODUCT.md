...
配信者はPC側の配信ソフト(OBS等)を操作しながら、本アプリをスマートフォンでバックグラウンド動作させる想定。Googleアカウントでサインイン→初回のみTikTok ID連携(LIVE Sidestage Analyticsへの登録)→ホーム画面で読み上げ開始/停止を操作、という一直線のフロー。Socket.IOでLIVE Sidestage Analyticsバックエンドとリアルタイム接続する。
