...
last_risk: LOW
last_reviewers: Gemini-3.7-Flash(TestCase review: Code Mode同時)
---
...
| TC-MTA-127 | iOS向け通常ビルドではdart-define未指定でもウェルカム画面にAppleログインボタンが出る | `api_client.dart`(`isAppleSignInConfigured`)/`welcome_screen.dart` | UI/正常 | iOS releaseビルド、`APPLE_SIGN_IN_IOS_ENABLED`未指定 | Googleログインの下に「Appleでサインイン」(`SignInWithAppleButton`)が表示される | Mac `~/bin/mac-build mobile-ios-run` 後、未ログイン状態でウェルカム画面を目視 | NOT RUN: 検証端末はMac+iPhone。WindowsセッションからMarionette MCP未接続 | 2026-09-12。`APPLE_SIGN_IN_IOS_ENABLED`既定true |
| TC-MTA-128 | Android向けビルドは従来どおりServices ID未設定ならAppleボタンを出さない | `api_client.dart`(`isAppleSignInConfigured`) | negative/回帰 | `flutter test`実行環境(非iOS)、`APPLE_SERVICES_ID`未指定 | `isAppleSignInConfigured`がfalse | `flutter test test/auth_provider_test.dart --plain-name "Services ID が未設定"` | PASS | |
...
- Apple Sign-InのauthorizationCode交換フロー本体は対象外。今回はiOSのボタン表示ゲート(`APPLE_SIGN_IN_IOS_ENABLED`既定true)のみ
