...
last_risk: MEDIUM
last_reviewers: Gemini(agy、Code Mode、medium)。大規模room固まる不具合修正(Batch01/02、CustomScrollView+ListPanelSliver化)
---
...
- TC-CT-022の「ログアウト」ボタンは存在確認のみ行い、タップ後に実際に`performLogout`が実行され`SessionController.logout()`まで到達するかは検証しない(既存踏襲、詳細は`worktree-mobile-contribution-breakdown-error`側のbaseline参照)
- バトル履歴タブの `RankingListTile` 呼び出し2箇所(`fetchBreakdown` 未指定)の `openTiktokProfile`(url_launcher)実呼び出し検証: テスト環境でurl_launcherをモックする既存パターンが無いため対象外
- TC-CT-017/018のpush反映の実機確認: 本worktreeには`.mcp.json`(Marionette MCP)・`adb`が未導入のためNOT RUN(コードレビューと`flutter test`で担保)
- `fetchGiftRankingShareUrl`(`lib/core/api_client.dart`)自体のFlutter unit test: 既存の慣行としてFlutter側では単体テストせず、analytics側のroute.integration.test.tsで契約を担保する
