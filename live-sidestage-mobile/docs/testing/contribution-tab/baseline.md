...
- `live-sidestage-mobile`: `flutter analyze` → PASS(2026-09-13、既存info 1件のみ) / `flutter test test/period_selector_bar_test.dart test/analytics_period_test.dart test/plan_gate_test.dart` → PASS(13/13) / Marionette・Pixel 7a → NOT RUN(Windows・adb PATH未設定)
- `live-sidestage-analytics`: `npm run typecheck` → PASS(2026-09-13) / 対象vitest → PASS(43/43)
