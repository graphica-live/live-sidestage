...
| TC-BH-022 | 「今日」を含む期間・カスタム範囲を表示中、Socket.IOで正常なbattle-history upsert pushを受信すると、REST再取得を待たず該当バトルの表示(進行中スコア等)が更新される | `BattleHistoryTab.build` + `BattleHistorySyncStore` | 正常/回帰 | 期間選択が今日を含む状態で、`BattleHistorySyncStore`が`canApply`な`chat:battle-history:upsert`を受信(version整合) | `containsToday`が`true`のため`store.getBattles()`が描画ソースに使われ、最新状態(新規または上書き)が反映される | コードレビュー(`build()`が`containsToday`条件下で`context.watch<BattleHistorySyncStore>().getBattles()`を`BattleSummary.tryParse`で復元して参照していることを確認) + `flutter test`/`flutter analyze` | PASS(コードレビュー確認、Codexレビューで検出されたHIGH不具合の修正) | Batch06で修正。修正前は`needsResync`時のみ`_load()`が呼ばれ、正常push受信時は画面が一切更新されない不具合があった |
| TC-BH-025 | 「今日」を含まない過去の期間・カスタム範囲を表示中は、battle-history push受信でもStoreが描画ソースにならず表示が汚染されない | `BattleHistoryTab.build`の`containsToday`ガード | 境界/negative | 過去のカスタム範囲(customRangeContainsNowがfalse)を選択中に、pushが届き`BattleHistorySyncStore`へ追加される | `containsToday`が`false`のため`storeBattles`は常に空扱いとなり、`allBattles`はRESTの`result?.battles`のまま(Storeの内容で選択中の範囲データが隠蔽されない) | コードレビュー(`build()`の`containsToday ? context.watch<BattleHistorySyncStore>().getBattles() : const []`を確認) + `flutter analyze`/`flutter test` | PASS | Batch02のCodex code-review finding(MEDIUM: battle-history pushが選択中の期間・カスタム範囲を無視してStoreへ無条件upsertされ、表示ソースが完全にStoreへ切り替わる)の修正。2026-09-12対応 |
...
| TC-BH-026 | FREE confirmed users see collection-limit notice at top of battle history | `FreePlanLimitNotice` / `PlanGate.showFreeBattleCollectionLimit` | normal | `isFree && !isFallback` | Full specified copy is shown; CTA `PROへアップグレード` uses Kosai c2; tap calls `openSubscriptionScreen` | `flutter test test/free_plan_limit_notice_test.dart test/plan_gate_test.dart` | PASS | Does not change fetch/display logic. Shown even when list is empty |
| TC-BH-027 | PRO/ULTRA, mobile beta, and fallback hide the notice | `PlanGate.showFreeBattleCollectionLimit` | negative | PRO / FREE+mobileBetaActive / AccountStatus.fallback | getter is false | `flutter test test/plan_gate_test.dart` | PASS | Hide during beta. Do not treat network-fallback FREE as confirmed |
