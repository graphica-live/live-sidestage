...
- **絞り込み:** 数値の範囲指定は`ChoiceChip`の横スクロール(コイン帯 すべて/1〜9/10〜99/100〜999/1000以上)。数値入力欄にするとキーボードが検索欄と競合する。**候補が値ではなく範囲を持つ場合、絞り込みは範囲の重なりで判定する**(下限だけ・上限だけを見ると価格違いのある候補が帯から漏れる)。
