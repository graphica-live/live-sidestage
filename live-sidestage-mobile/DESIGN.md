...
- **検索:** 未入力なら全件。入力のたびにローカルで即時絞り込む(候補は数百件なのでサーバーへ問い合わせ直さない)。`autofocus`はしない――未入力の全件を見たいときにキーボードが一覧を半分隠すため。クリアボタンは入力があるときだけ出す。
- **絞り込み:** 数値の範囲指定は`ChoiceChip`の横スクロール(コイン帯 すべて/1〜9/10〜99/100〜999/1000以上)。数値入力欄にするとキーボードが検索欄と競合する。
