...
採用元: 計画「モバイル ログイン後オンボーディング」方向（スライド3枚 + TikTok連携 + 確認シート）。
共通トークン・換算規則は `../_kosai-tokens.md` を正本とする。

comp caption:
> ログイン後・Streamer未登録時。紹介3枚はスキップ可、最終ページのTikTok ID連携は必須。
...
| 要素 | 仕様 |
|---|---|
| スキップ | 紹介ページ0–2のみ。14dp w600 / sub `#7C7286` / 高さ48dp / 左寄せ。ページ3では同じ幅の空 `SizedBox` でレイアウトを保つ |
...
| 2 | アイコン | 白 44dp。0=`Icons.record_voice_over` / 1=`Icons.card_giftcard` / 2=`Icons.graphic_eq` |
| 3 | 余白 | 24dp |
...
- 0 見出し「画面を見ずに、声で聞く」 / 本文「TikTok LiveのコメントをVOICEVOXが読み上げます。配信ソフトから目を離さずに応答できます。」
- 1 見出し「ギフトが届いたら、音でわかる」 / 本文「受け取ったギフトに合わせた効果音を鳴らせます。配信中も片手で対応表を触れます。」
- 2 見出し「画面オフでも、途切れない」 / 本文「読み上げと効果音はバックグラウンドでも続きます。開始してから60分ライブが無ければ自動で停止します。」
...
| 2 | 余白 | 8dp |
| 3 | 歓迎文 | 13.5dp / sub 「ようこそ、{userName}さん。配信を読み上げるTikTok IDを連携してください。」 |
| 4 | 余白 | 24dp |
...
- 横スワイプ: PageView。ページ3から先へは進めない
- スキップ: ページ3へジャンプ（アニメーションあり）
- 次へ: 次ページ。ページ2では連携ページへ
