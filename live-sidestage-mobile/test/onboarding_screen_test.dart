...
import 'package:live_sidestage_mobile/core/intro_onboarding_store.dart';
...
Future<IntroOnboardingStore> _pump(

  WidgetTester tester,

  SessionController controller, {

  OnboardingPhase phase = OnboardingPhase.postLoginLink,

}) async {

  final intro = IntroOnboardingStore()..debugSeed(completed: false);
  ...
  await tester.pumpWidget(
    ...
    MultiProvider(
      ...
      providers: [
        ...
        ChangeNotifierProvider<IntroOnboardingStore>.value(value: intro),

      ],

      child: MaterialApp(home: OnboardingScreen(phase: phase)),
...
void main() {
  ...
  testWidgets('ログイン前紹介でスキップすると紹介完了になる', (tester) async {

    final intro = await _pump(tester, _controller(), phase: OnboardingPhase.preLoginIntro);

    await tester.pumpAndSettle();



    expect(find.text('画面を見ずに、コメントがわかる'), findsOneWidget);
    ...
    expect(intro.completed, isTrue);

    expect(find.text('TikTokアカウントの連携'), findsNothing);

  });



  testWidgets('次へでログイン前紹介スライドを順に進める', (tester) async {
    ...
    expect(find.text('ギフトが、音と演出に変わる'), findsOneWidget);
    ...
    expect(find.text('配信画面のまま、読み上げも効果音も'), findsOneWidget);
    ...
    expect(find.text('応援してくれた人を、見逃さない'), findsOneWidget);
    ...
    expect(find.text('あのバトルを、あとから振り返る'), findsOneWidget);
  ...
  testWidgets('preview中は確認中ローディング行と入力disabled', (tester) async {

    final api = _FakeApi()..holdPreview = Completer<void>();
    ...
    expect(find.text('TikTokアカウントを確認しています…'), findsOneWidget);
